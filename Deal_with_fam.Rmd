...
parents<- test[relationship %in% c(5:19)]
parents[, edu := ifelse(edu == 99, NA, parents$edu)][, qser_no:= paste0(qtype,  ser_no)]
...
detect_4<- function(x,y,z,u){
  for (i in unique(parents$ID)){
  if(length(parents[ ID ==i &survey_year ==x ]$birthyear)==0 | length(parents[ ID ==i &survey_year ==y]$birthyear) ==0 |length(parents[ ID ==i &survey_year ==z ]$birthyear)==0 | length(parents[ ID ==i &survey_year ==u]$birthyear) ==0 |length(parents[ ID ==i &survey_year ==x ]$gender)==0 | length(parents[ ID ==i &survey_year ==y]$gender) ==0 |length(parents[ ID ==i &survey_year ==z ]$gender)==0 | length(parents[ ID ==i &survey_year ==u]$gender) ==0 |length(parents[ ID ==i &survey_year ==x ]$edu)==0 | length(parents[ ID ==i &survey_year ==y]$edu) ==0 |length(parents[ ID ==i &survey_year ==z ]$edu)==0 | length(parents[ ID ==i &survey_year ==u]$edu) ==0){next}
  
   else if(length(parents[ ID ==i &survey_year ==x ]$birthyear)>1 | length(parents[ ID ==i &survey_year ==y]$birthyear) >1 |length(parents[ ID ==i &survey_year ==z ]$birthyear)>1 | length(parents[ ID ==i &survey_year ==u]$birthyear) >1 |length(parents[ ID ==i &survey_year ==x ]$gender)>1 | length(parents[ ID ==i &survey_year ==y]$gender) >1 |length(parents[ ID ==i &survey_year ==z ]$gender)>1 | length(parents[ ID ==i &survey_year ==u]$gender) >1 |length(parents[ ID ==i &survey_year ==x ]$edu)>1 | length(parents[ ID ==i &survey_year ==y]$edu) >1 |length(parents[ ID ==i &survey_year ==z ]$edu)>1 | length(parents[ ID ==i &survey_year ==u]$edu) >1){ parents[ID ==i, age_acc:= FALSE][ID ==i, gender_acc:= FALSE][ID ==i, edu_acc:= FALSE] }
     else{
  parents[ID ==i, age_acc:= ifelse( (parents[ ID ==i &survey_year ==u ]$birthyear  - parents[ ID ==i &survey_year ==z]$birthyear) %in% c(-1:2) & (parents[ ID ==i &survey_year ==z ]$birthyear  - parents[ ID ==i &survey_year ==y]$birthyear) %in% c(-1:2) & (parents[ ID ==i &survey_year ==y ]$birthyear  - parents[ ID ==i &survey_year ==x]$birthyear) %in% c(-1:2), TRUE, FALSE)]
  parents[ID ==i, gender_acc := ifelse(parents[ ID ==i &survey_year ==z ]$gender  != parents[ ID ==i &survey_year ==u]$gender | parents[ ID ==i &survey_year ==z ]$gender != parents[ ID ==i &survey_year ==y]$gender | parents[ ID ==i &survey_year ==y ]$gender  != parents[ ID ==i &survey_year ==x ]$gender , FALSE, TRUE) ]
  parents[ID ==i, edu_acc :=  ifelse ( abs(parents[ ID ==i &survey_year ==u ]$edu  - parents[ ID ==i &survey_year ==z]$edu) >=4|abs(parents[ ID ==i &survey_year ==z ]$edu  - parents[ ID ==i &survey_year ==y]$edu) >=4 | abs(parents[ ID ==i &survey_year ==y ]$edu  - parents[ ID ==i &survey_year ==x]$edu) >=4, FALSE, TRUE)]
     }}
}

detect_4(1996,1999,2003,2007)

detect_3 <- function(y,z,u){
  for (x in unique(parents$ID)){
  if(length(parents[ ID ==x &survey_year ==y ]$birthyear)==0 | length(parents[ ID ==x &survey_year ==z]$birthyear) ==0 |length(parents[ ID ==x &survey_year ==u ]$birthyear)==0 |length(parents[ ID ==x &survey_year ==y ]$gender)==0 | length(parents[ ID ==x &survey_year ==z]$gender) ==0 |length(parents[ ID ==x &survey_year ==u ]$gender)==0 |length(parents[ ID ==x &survey_year ==y ]$edu)==0 | length(parents[ ID ==x &survey_year ==z]$edu) ==0 |length(parents[ ID ==x &survey_year ==u ]$edu)==0){next}
  
     else if(length(parents[ ID ==x &survey_year ==y ]$birthyear)>1 | length(parents[ ID ==x &survey_year ==z]$birthyear) >1 |length(parents[ ID ==x &survey_year ==u ]$birthyear)>1  |length(parents[ ID ==x &survey_year ==y ]$gender)>1 | length(parents[ ID ==x &survey_year ==z]$gender) >1 |length(parents[ ID ==x &survey_year ==u ]$gender)>1  |length(parents[ ID ==x &survey_year ==y ]$edu)>1 | length(parents[ ID ==x &survey_year ==z]$edu) >1 |length(parents[ ID ==x &survey_year ==u ]$edu)>1 ){ parents[ID ==x, age_acc:= FALSE][ID ==x, gender_acc:= FALSE][ID ==x, edu_acc:= FALSE] }
  
     else{
  parents[ID ==x, age_acc:= ifelse((parents[ ID ==x &survey_year ==u ]$birthyear  - parents[ ID ==x &survey_year ==z]$birthyear)%in% c(-2:2) & (parents[ ID ==x &survey_year ==z ]$birthyear  - parents[ ID ==x &survey_year ==y]$birthyear) %in% c(-2:2), TRUE, FALSE)]
  parents[ID ==x, gender_acc := ifelse( parents[ ID ==x &survey_year ==u ]$gender != parents[ ID ==x &survey_year ==z]$gender | parents[ ID ==x &survey_year ==z ]$gender  != parents[ ID ==x &survey_year ==y]$gender , FALSE, TRUE) ]
  parents[ID ==x, edu_acc :=  ifelse ( abs(parents[ ID ==x &survey_year ==u ]$edu  - parents[ ID ==x &survey_year ==z]$edu) >=4 | abs(parents[ ID ==x &survey_year ==z ]$edu  - parents[ ID ==x &survey_year ==y]$edu) >=4, FALSE, TRUE)]
     }}
}


detect_3(1996,1999,2003)
detect_3(1996,1999,2007)
detect_3(1996,2003,2007)
detect_3(1999,2003,2007)


detect<- function(u,v){
for (x in unique(parents$ID)){
  if(length(parents[ ID ==x &survey_year ==u ]$birthyear)==0 | length(parents[ ID ==x &survey_year ==v]$birthyear) ==0 |length(parents[ ID ==x &survey_year ==u ]$gender)==0 | length(parents[ ID ==x &survey_year ==v]$gender) ==0 |length(parents[ ID ==x &survey_year ==u ]$gender)==0 |length(parents[ ID ==x &survey_year ==v ]$edu)==0 | length(parents[ ID ==x &survey_year ==u]$edu) ==0 |length(parents[ ID ==x &survey_year ==v ]$edu)==0){next}
  
   else if(length(parents[ ID ==x &survey_year ==u ]$birthyear)>1 | length(parents[ ID ==x &survey_year ==v]$birthyear) >1 |length(parents[ ID ==x &survey_year ==u ]$gender)>1 | length(parents[ ID ==x &survey_year ==v]$gender) >1 |length(parents[ ID ==x &survey_year ==u ]$edu)>1 | length(parents[ ID ==x &survey_year ==v]$edu) >1 ){ parents[ID ==x, age_acc:= FALSE][ID ==x, gender_acc:= FALSE][ID ==x, edu_acc:= FALSE] }
     else{
  parents[ID ==x, age_acc:= ifelse( (parents[ ID ==x &survey_year ==v ]$birthyear  - parents[ ID ==x &survey_year ==u]$birthyear) %in% c(-1:2), TRUE, FALSE)]
  parents[ID ==x, gender_acc := ifelse( parents[ ID ==x &survey_year ==v ]$gender != parents[ ID ==x &survey_year ==u]$gender , FALSE, TRUE) ]
  parents[ID ==x, edu_acc :=  ifelse ( abs(parents[ ID ==x &survey_year ==v ]$edu  - parents[ ID ==x &survey_year ==u]$edu) >=4, FALSE, TRUE)]
     }}
}


detect_2<- function(x, y, z,u ){
  detect_4(x,y,z,u)
  detect_3(x,y,z)
  detect_3(y,z,u)
  detect_3(x,y,u)
...
#排列組合出所有組合 
detect_2(1996,1999,2003,2007)



problems <- unique(parents[age_acc ==FALSE | gender_acc ==FALSE | edu_acc ==FALSE]$qser_no) 
...
problems <- unique(parents[age_acc ==FALSE | gender_acc ==FALSE]$qser_no) # 先不看教育
problem_parents <- parents[ qser_no %in% problems][edu!=90] 
...
prepare_kmeans<- function(x){
 problem_parents[ qser_no == x, birthyear:=  survey_year - age - 1911][qser_no ==x, k_gender:= 7*gender][qser_no==x, k_edu:=5*edu][qser_no==x, k_birthyear:=3*birthyear]
}

# get k means, cluster 數量取最多的那年的個數
get_kmeans <- function(x){
kmeans.cluster <- kmeans(problem_parents[qser_no ==x, .SD , .SDcols = c("k_birthyear","k_gender","k_edu", "relationship")], centers= sum(problem_parents[qser_no ==x]$survey_year == getmode(problem_parents[qser_no ==x]$survey_year)), algorithm="MacQueen", trace = TRUE)
problem_parents[qser_no ==x ,group:= paste(qser_no,kmeans.cluster$cluster, sep = "_")]
}



for (i in unique(problem_parents$qser_no)){
  prepare_kmeans(i)
}
...
s<-one2one(problem_parents, "ID", "group")
ss<- as.data.table(as.matrix(s))
unsolve<- unique(parents[ID %in% unique(ss$V1)]$qser_no)
solve<- setdiff( problems, unsolve)
# 剩下4個的qser_no

parents_unsolve <- parents[qser_no %in% unsolve]$ID
...
```{r}
# same generations
brothers <- test[relationship %in% c(20:29)]
