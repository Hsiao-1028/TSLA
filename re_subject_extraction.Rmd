...
demo_89 <- df_89[ , c("qtype", "ser_no", "survey_year","rsex", "rage","reduc","ethnicty","marstat")]
...
# 96年多了一個general （總合來說你覺的你今年的身體住框如和）
```
...
dt_99[, rage:= survey_year - birthyear][, grep("^i.", colnames(dt_99)) :=NULL ]
...
dt_03o[,reduc := educ][,ethnicty := ethc][, grep("^i.", colnames(dt_03o)) :=NULL ][, birthyear := 2003 -1911- age][, c('sex','educ', 'ethc') :=NULL]
...
dt_03y[, grep("^i.", colnames(dt_03o)) :=NULL ][, birthyear := 2003 -1911- age]
...
dt_07[, reduc := ethc07][, age:= age07s][, ethnicty := ethc07][, grep("^i.", colnames(dt_07)) :=NULL ][, c('educ07', 'ethc07','age07s'):=NULL][, birthyear := 2007 -1911- age]
